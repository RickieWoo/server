...
if [ "$JDK_VERSION" == '1' ];then
  ...
  ln -s /usr/java/jdk${jdk16_version} /usr/java/default
  sed -i "s@^export JAVA_HOME=.*@export JAVA_HOME=/usr/java/jdk${jdk16_version}@" /etc/init.d/tomcat
  ...
  /etc/init.d/tomcat stop > /dev/null 2>&1;[ -n "ps -ef | grep java | grep -v grep" ] && killall java > /dev/null 2>&1;rm -rf /usr/java/default
  ln -s /usr/java/jdk${jdk17_version} /usr/java/default
  sed -i "s@^export JAVA_HOME=.*@export JAVA_HOME=/usr/java/jdk${jdk17_version}@" /etc/init.d/tomcat
  /etc/init.d/tomcat start > /dev/null 2>&1
  echo
  ...
  /etc/init.d/tomcat stop > /dev/null 2>&1;[ -n "ps -ef | grep java | grep -v grep" ] && killall java > /dev/null 2>&1;rm -rf /usr/java/default
  ln -s /usr/java/jdk${jdk18_version} /usr/java/default
  sed -i "s@^export JAVA_HOME=.*@export JAVA_HOME=/usr/java/jdk${jdk18_version}@" /etc/init.d/tomcat
  /etc/init.d/tomcat start > /dev/null 2>&1
  echo
