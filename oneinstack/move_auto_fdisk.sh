...
check_disk() {
  > $LOCKfile
  for i in `fdisk -l | grep "Disk" | grep "/dev" | awk '{print $2}' | awk -F: '{print $1}' | grep -E "vd|sd"`
  do
...
check_os
check_disk
[ -n "`ps -ef | grep mysql | grep -v grep`" ] && { service mysqld stop > /dev/null 2>&1; DB_flag=1; }
[ -n "`ps -ef | grep nginx | grep -v grep`" ] && { killall nginx; Nginx_flag=1; }
[ -n "`ps -ef | grep php-fpm | grep -v grep`" ] && { service php-fpm stop > /dev/null 2>&1; PHP_flag=1; }
[ -n "`ps -ef | grep httpd | grep -v grep`" ] && { service httpd stop > /dev/null 2>&1; Apache_flag=1; }
[ -n "`ps -ef | grep tomcat | grep -v grep`" ] && { service tomcat stop > /dev/null 2>&1; Tomcat_flag=1; }
mv /data /root
main
df -h
mv /root/data/* /data
[ "$DB_flag" == '1' ] && service mysqld start > /dev/null 2>&1
[ "$Nginx_flag" == '1' ] && service nginx start > /dev/null 2>&1
[ "$PHP_flag" == '1' ] && service php-fpm start > /dev/null 2>&1
[ "$Apache_flag" == '1' ] && service httpd start > /dev/null 2>&1
[ "$Tomcat_flag" == '1' ] && service tomcat start > /dev/null 2>&1
[ -d /data/wwwroot ] && rm -rf /root/data
